...
class TestTriNet {
	...
	private ComportamentalFSM c1;
	private ComportamentalFSM c2;
	private ComportamentalFSM c3;	
	
	private ComportamentalState s10;
	private ComportamentalState s11;
	...
	private ComportamentalTransition t1a;
	private ComportamentalTransition t1b;
	private ComportamentalTransition t1c;
	...
	private CFSMnetwork initialize() {
		//AUTOMATA C1
		c1 = new ComportamentalFSM("C1");
		s10 = new ComportamentalState("10");
		s11 = new ComportamentalState("11");
		c1.insert(s10);
		c1.insert(s11);
		c1.setInitial(s10);
		...
		l2 = new Link("L1", c2, c1);
		l2 = new Link("L2", c2, c3);
		l3 = new Link("L3", c3, c1);	
		
		t3a = new ComportamentalTransition("t1a", s10, s11, e1, l1, new ObservableLabel(), new RelevantLabel());
		t3b = new ComportamentalTransition("t1b", s11, s10, e2, l3, new ObservableLabel(), new RelevantLabel());
		t3c = new ComportamentalTransition("t1c", s10, s11, new ObservableLabel(), new RelevantLabel("f"));
		
		t2a = new ComportamentalTransition("t2a", s20, s21, outEventsToHashMap(new Event[]{e1, e3}, new Link[]{l1, l2}),
				new ObservableLabel("o1"), new RelevantLabel());
		t2b = new ComportamentalTransition("t2b", s21, s20, outEventsToHashMap(new Event[]{e1}, new Link[]{l1}),
				new ObservableLabel("o2"), new RelevantLabel());
		
		t3a = new ComportamentalTransition("t3a", s30, s31, outEventsToHashMap(new Event[]{e2}, new Link[]{l3}),
				new ObservableLabel(), new RelevantLabel());
		t3b = new ComportamentalTransition("t3b", s31, s30, e3, l2, new ObservableLabel(), new RelevantLabel());
		t3c = new ComportamentalTransition("t3c", s31, s31, e3, l2, new ObservableLabel(), new RelevantLabel("f3"));
		...
		listLink.add(l1);  listLink.add(l2); listLink.add(l3);
	...
	private HashMap<String, ComportamentalState> statesToHashMap(ComportamentalState stateC1, ComportamentalState stateC2, ComportamentalState stateC3) {
		HashMap<String , ComportamentalState> map = new HashMap<String , ComportamentalState>();
		map.put(c1.id(), stateC1);
		map.put(c2.id(), stateC2);
		map.put(c3.id(), stateC3);
		return map;
	}
	
	private HashMap<Event, Link> outEventsToHashMap(Event[] events, Link[] links) {
		HashMap<Event, Link>  map = new HashMap<Event, Link> ();
		for(int i=0; i<links.length; i++)
			map.put(events[i], links[i]);
		return map;
	}
