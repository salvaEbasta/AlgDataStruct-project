...
public class MultipleRelRegexBuilder <S extends State, T extends Transition<S>> extends Algorithm<HashMap<S, String>>{
	...
	private boolean findParallelTransitions(LinkedList<T> transitions, HashMap<S, LinkedList<T>> markings) throws Exception{
		transitions.clear();
	...
	private boolean findOneWayPathMarked(LinkedList<T> transitions, HashMap<S, LinkedList<T>> markings, S mark) throws Exception {
		transitions.clear();
		for(S s : N.states()) {
			//log.info("State : "+s);
	...
	private boolean findMarkedParallelTransitions(LinkedList<T> transitions, HashMap<S, LinkedList<T>> markings, S mark) throws Exception {
		for(S state : markings.keySet()) {
